...
61. As a ChatGPT Desktop user, I want every protected Send to leave a local, raw-free trace from interception through final outcome, so that the product can prove one complete protection path rather than merely reporting that its separate components are enabled.
62. As a maintainer, I want a reference-only input source that cannot target Codex or ChatGPT, so that local acceptance can exercise the resident hook path without creating a bypass in normal AI-app protection.
63. As a Windows user, I want all setup, recovery, and local readiness actions required for protection to start automatically, so that I do not have to know commands, prerequisites, or their order.
64. As a Windows user, I want to see the current stage, progress, outcome, and next safe action for every protection readiness action, so that a long-running check never looks like a hung application.
...
- Release acceptance for a pinned fingerprint has two layers. A deterministic local reference-composer test exercises the actual Windows hook, UI Automation, Send control, overlay foreground handling, text replacement, and replay without a cloud service. A repeatable live desktop contract run verifies the same trace for the pinned ChatGPT Desktop fingerprint. The release cannot claim `protected` for that fingerprint without both layers passing for the shipped build.
- **Reference-only hook input source.** The local acceptance layer uses one compiled-in `reference-composer` profile, not a user-configured or persisted AI profile. It is created only by the reference-composer host in an explicit release-smoke/acceptance scope. It is never listed in onboarding, tray UI, policy import, profile storage, or normal command handling, and the normal resident runtime has no transition that can enable it.
- The reference source is authorized by a non-serializable, per-process capability created with the local reference window. The capability is bound to that host's process, normalized root window, UI thread and fresh in-memory nonce. Every gesture revalidates those bindings. A profile identifier, `dwExtraInfo` marker, command-line switch, window title, or any disk value alone is never authorization. Requests for Codex, ChatGPT, a custom profile, a different process/root window/thread, or a stale/missing capability are rejected before classification as `reference_source_unavailable` and produce no Send operation.
- The source enters the hook-owner callback adapter before selected-target classification and suppression. The native `WH_KEYBOARD_LL`/`WH_MOUSE_LL` callbacks and the reference source call the same internal captured-gesture entry; this entry owns binding recognition, selected/unrelated decision, suppression, snapshot capture, attempt creation, and deferred operation scheduling. The reference source must not call a controller, queue, resident runner, overlay, writer, replay helper, or trace publisher directly. Generic injected Windows input remains ignored by the native callbacks, so production replay cannot be recaptured as a new protected Send.
- The reference source may construct only the compiled-in local target identity and cannot coexist as an alternate route for a Codex/ChatGPT snapshot. The immutable resident snapshot remains the sole authority for its temporary acceptance state; the tray only renders published state and cannot activate the source. Leaving the acceptance scope, a hook/identity mismatch, timeout, or any cleanup failure invalidates the capability and removes the reference profile atomically.
- Reference-composer approval uses the production `WindowsConfirmationOverlay`, including its foreground-validation path. Deterministic dialog automation, when needed, is a production-owned lifecycle seam that runs after foreground validation; it cannot forge overlay or trace transitions. The only send is through the target-owned keyboard/pointer replay boundary. A partial input injection or modifier-release failure is terminal `replay_indeterminate`, never `sent_safely`.
- The reference-composer acceptance run is raw-free and cloud-free. It validates the complete terminal resident trace and runs twice in one process to prove deterministic cleanup of hooks, dispatcher, windows, capability state, temporary storage and pending work. Source rejection, missing trace stages, invalid order, cancellation, target change, overlay failure, write failure, replay uncertainty, or cleanup failure are fail-closed and cannot count as a successful acceptance run. This local proof does not claim compatibility with a real Codex or ChatGPT surface; that remains the separately pinned live contract proof.
- **Automatic operational orchestration.** Every action required to make Code Sanitizer operational on a desktop - first-run setup, selected-profile verification, resident activation/recovery, local readiness checks, and any local prerequisite for release/CI acceptance - must be started by the resident application at the appropriate lifecycle point. The user must not have to discover a command, choose an execution order, or manually start a technical prerequisite. Explicit user consent is still permitted only for security decisions such as emergency bypass or selecting a configured Send binding.
- **One action-lifecycle owner.** A resident operational-action module owns each action's correlation identifier, current stage, terminal outcome, cancellation, and retry eligibility. Its state is `idle -> running(stage) -> succeeded | failed(reason) | cancelled`; a failed or cancelled prerequisite is fail-closed for protected Send. The tray and status windows only project this published resident state and do not keep local readiness flags or independently decide that protection is ready.
- **Readable progress, never an opaque wait.** While an operational action is running, the UI must identify the action, current stage, automatic or user-required input, elapsed time, cancellation availability, terminal outcome, and one safe next action. The window remains usable: it can be moved, closed when no critical decision is pending, and must not block ordinary ChatGPT/Codex navigation or clipboard use. A state labelled `checking` without a named stage and visible result is not sufficient.
- **Raw-free operational journal.** The same lifecycle owner writes a durable, bounded local journal for every operational action. Each record contains a correlation identifier, action kind, lifecycle transition, stage, outcome/failure code, elapsed duration, attempt count, and installed build identity. It must not contain prompt text, detected values, mappings, file paths, window titles, control names, local addresses, or exception messages. The journal is the first diagnostic source for support and must allow a maintainer to determine the failed stage unambiguously from raw-free codes.
- **Release/CI versus post-install readiness.** The complete multi-scenario reference-composer matrix is release/CI acceptance evidence and must not run as an unexplained tray command during ordinary setup. The installed product instead performs a short, explicit local readiness action automatically and reports its own result. A verified current-build target may enter the resident `protected_send_active` state from active-target verification plus local readiness alone. The separately recorded reference/live evidence remains required for the distinct release-compatibility claim `protected`; stale or missing release evidence is diagnostic and CI state, not a hidden resident admission gate.
- **Manual-acceptance admission gate.** New manual desktop/ChatGPT acceptance runs are suspended until automatic orchestration, detailed stage UI, the durable raw-free journal, the separation of local readiness from release/CI acceptance, and the same-process tray integration proof are implemented. Before that gate is complete, user observations may be retained as diagnostics only; they cannot mark a feature accepted, protected, or release-ready.
...
- The installed tray app must expose one `Set up prompt protection` action without requiring the user to copy `dotnet run` commands. It must determine whether the focused supported composer belongs to Codex Desktop or ChatGPT Desktop and verify that active profile; normal onboarding must not require a manual app choice. Console commands may remain diagnostics only.
- Codex Desktop and ChatGPT Desktop are internal Windows surface variants, not a product choice exposed during setup. After installation or a retry, the resident workflow must automatically enter one durable `waiting_for_focus` stage, wait for any compatible OpenAI Desktop composer, and persist only the detected verified target. A previously protected but inactive variant must not complete the current setup or suppress the waiting workflow.
- Automatic startup, tray retry, and installer launch must invoke the same setup lifecycle. The lifecycle publishes `waiting_for_focus`, discovery, verification, activation, and one terminal result to the resident-state projection and raw-free operational journal. There must be no alternate path that silently completes because another profile happens to be stored.
- User-facing status must identify the protected surface as `OpenAI Desktop` and explain the current setup stage and safe next action. Internal Codex/ChatGPT variant identifiers are raw-free diagnostic data and must not require the user to distinguish process names or package identities.
...
- Protected-Send trace tests must prove each legal transition sequence for safe Send, sensitive confirmation and approval, cancellation, target change, overlay-foreground refusal, text-write failure, replay failure, stale generation, and duplicate gesture. They must reject missing, duplicated, or out-of-order transitions without using timers or a cloud service.
- The local reference-composer acceptance test must drive the same Windows hook, UI Automation, overlay, text-write, and keyboard/mouse replay mechanisms as the tray application. It must prove that raw text is not sent after cancellation or a failed stage, and that only verified sanitized text is sent after approval.
- Reference-source tests must prove that Codex/ChatGPT and all persisted/custom profiles cannot activate the source; a wrong process, root window, UI thread, nonce, or expired acceptance scope is rejected before suppression. They must also prove that the reference source and native callback reach the same captured-gesture entry, while generic replay injection is ignored by the native callback.
- Reference-composer acceptance tests must use the production overlay lifecycle and target-owned keyboard/pointer replay boundary, inspect the complete raw-free terminal trace, report partial injection as `replay_indeterminate`, and run the fixture twice in one resident process without timers, cloud access, raw prompts, or leaked handles/threads.
- The pinned ChatGPT Desktop contract run must record the application compatibility fingerprint and a raw-free protected-Send trace for the configured keyboard path. Mouse Send is included only when the pinned fingerprint has verified pre-action evidence; unsupported programmatic UI Automation Invoke remains explicitly unsupported.
...
- Operational-action tests must drive automatic start, stage transition, cancellation, retry, and terminal outcomes through one deterministic lifecycle seam. They must assert that selected-app Send remains fail-closed while a required action is incomplete, with no timers, foreground focus, or live cloud submission.
- UI tests must prove that each running operational action exposes a named stage, progress/result, and safe next action; an opaque `checking` state is rejected. They must also prove the status window remains usable and does not suppress unrelated clipboard, navigation, or editing input.
- Journal tests must prove that every operational-action outcome has one durable, correlated raw-free record; they must reject prompt text, sensitive values, mappings, paths, window/control names, local addresses, and exception messages.
- Release/CI tests must prove that the full reference-composer matrix is invoked through the release/CI acceptance path, while the installed resident application runs only the short local readiness path. A same-process tray integration test must run with an active native hook and prove a readiness action reaches a terminal state without deadlock, stale hook state, or a hung UI.
- Manual desktop acceptance evidence is invalid until the operational-action admission gate has deterministic proof for automatic orchestration, detailed UI state, raw-free diagnostics, release/CI separation, and same-process tray integration.
